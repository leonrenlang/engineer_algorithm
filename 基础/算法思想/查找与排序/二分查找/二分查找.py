# 功能/写法: 基础功能   重复返回左边界  重复返回右边界  插入位置i（a[:i]<target)  插入位置i(a[:i]<=target)
# 基础写法        √
# 左边界写法      √       √                                    √
# 右边界写法      √                         √
# 右边界写法+1    ×                                                                         √
...
# 算法思想
# 1. 给定一个有序列表，查找指定值
# 2. 二分查找就是不断判断指定返回内中间的值和指定值的大小关系，从而缩小范围
...
def binary_search_left_bound(nums, target):
    # def binary_search_left_insert(nums, target):
...
def binary_search_right_insert(nums, target):
    if len(nums) == 0: return -1
    left, right = 0, len(nums)
    while left < right:
        mid = (left + right) // 2
        if nums[mid] > target:
            ...
            left = mid + 1
    return left  # python 官方的bisect_right是直接返回left
